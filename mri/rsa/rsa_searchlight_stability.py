...
for space in spaces:
			...
			for subj in subjects:
				...
				for cond in ['time', 'dist', 'dots', 'lumin']:
					if cond not in conditions:
						fmri_data = fmri_data[
							fmri_data.sa.targets != cond]  # remove cross from the conditions (targets)
				...
				niimg.to_filename(opj(result_dir,
				                      subj_str + '_space-' + space + '_mask-' + roi + file_suffix + 'RDM_location_searchlight.nii.gz'))

				# save individual results
				with open(opj(result_dir, subj_str + '_space-' + space + '_mask-' + roi + file_suffix + 'roiData.pkl'),
				          'wb+') as f:
				...
				ax[1].set_title('Most stable dissimilarity pattern: {:.2f}'.format(
					np.mean(mean_consistency[mean_consistency.argmax()])))
				...
				rdms_cummulative_allSubj[roi] = rdms_cummulative_allSubj[
					                                roi] + rdm_data  # save current RDM for group level
				max_cons_allSubj[roi].append(
					mean_consistency[mean_consistency.argmax()])  # save current rdm consistency for group level
				...
				ax[2].hist(slres_tdsm.samples[0], normed=False, bins=10, color='SkyBlue')
				...
				if save_figs:
					plt.savefig(opj(result_dir,
					                subj_str + '_space-' + space + '_mask-' + roi + file_suffix + 'RDM_location_searchlight_new.png'),
					            dpi=300)

				plt.close()

				rdm_location_img_list.append(opj(result_dir,
				                                 subj_str + '_space-' + space + '_mask-' + roi + file_suffix + 'RDM_location_searchlight.png'))

			if plot_html:
				create_html_report(rdm_location_img_list,
				                   opj(group_result_dir,
				                       'RDM_space-' + space + '_mask-' + roi + file_suffix + 'searchlight_location.html'),
				                   title='Most stable RDM and location for mask: %s' % roi)

		# save
		with open(opj(group_result_dir, 'RDM_space-' + space + file_suffix + 'searchlight_mean.pkl'),
		          'wb+') as f:
			cPickle.dump([rdms_allSubj, rdms_cummulative_allSubj, max_cons_allSubj, mean_cons_allSubj], f)
			f.close()
	...
	if group_level:
		...
		with open(opj(group_result_dir, 'RDM_space-' + space + file_suffix + 'searchlight_mean.pkl'),
		          'rb+') as f:
			rdms_allSubj, rdms_cummulative_allSubj, max_cons_allSubj, mean_cons_allSubj = cPickle.load(f)
			f.close()
		...
		df.to_csv(opj(group_result_dir, 'RDM_space-MNI152NLin2009cAsym_searchlight_mean.csv'), sep='\t',
		          columns=['subject', 'roi', 'condition_1', 'condition_2', 'dissimilarity', 'consistency_max',
		                   'consistency_mean'],
		          index=False)
		...
		if (len(masks) % 2) == 0:
			...
			fig, ax = plt.subplots(1, len(masks), figsize=(30, 10), squeeze=False)
		...
		for roi_enum, roi in enumerate(masks):
			...
			if roi_enum == 0:
				...
				ax[subplot_row, subplot_col].set_yticklabels(mlabels, fontdict=None, minor=False, fontsize=12)

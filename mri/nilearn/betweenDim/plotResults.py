...
for space in output_space:
	...
	if group_level:
		...
		# plotting
		fig, ax = plt.subplots(int(len(conditions)/2), 2, figsize=(30, 20))
		plt.tight_layout(5)
		...
		for cond_enum, condition in enumerate(cbar):
			...
			if cond_enum < int(len(conditions)/2):
				subplot_row = cond_enum
				subplot_col = 0
			else:
				subplot_row = cond_enum - int(len(conditions)/2)
				subplot_col = 1
			...
			for roi in roi_names:
				...
				if 'lh' in roi:
					...
					# wilcoxon
					# subjs_data_lh = [x - 0.5 for x in subjs_data_lh]
					# stats, p = wilcoxon(subjs_ci_data_lh)
					...
					# wilcoxon
					# subjs_data_rh = [x - 0.5 for x in subjs_data_rh]
					# stats, p = wilcoxon(subjs_ci_data_rh)
			...
			# if multiple comparisons is true
			if correct_multiple_comparisons:
				subjs_p_data_lh_corr = list(multipletests(subjs_p_data_lh, alpha=0.05, method='hs')[1])
				subjs_p_data_rh_corr = list(multipletests(subjs_p_data_rh, alpha=0.05, method='hs')[1])
			else:
				subjs_p_data_lh_corr = subjs_p_data_lh
				subjs_p_data_rh_corr = subjs_p_data_rh
			...
			autolabel(lh, subjs_ci_data_lh, subjs_p_data_lh_corr, ax[subplot_row, subplot_col], "left")
			autolabel(rh, subjs_ci_data_rh, subjs_p_data_rh_corr, ax[subplot_row, subplot_col], "right")

...
for space in spaces:
    ...
    for subj in subjects:
        ...
        for roi_enum, roi in enumerate(masks):
            ...
            mtgs = mean_group_sample(['targets', 'chunks'])       # create the mean of all trials
            mtds = mtgs(fmri_data)
            dsm = rsa.CDist(pairwise_metric='correlation', pairwise_metric_kwargs={'center_data': True}, auto_train=True)
            res = dsm(mtds)
            ...
            for i in range(0, 9):
                if i == 1:
                    dm_matrix[0, 1] = res.samples[i]
                    dm_matrix[1, 0] = res.samples[i]
                elif i == 2:
                    dm_matrix[2, 0] = res.samples[i]
                    dm_matrix[0, 2] = res.samples[i]
                elif i == 5:
                    dm_matrix[2, 1] = res.samples[i]
                    dm_matrix[1, 2] = res.samples[i]
            res.samples = dm_matrix
            ...
            # plotting
            rdm_plot = ax[subplot_row, subplot_col].imshow(res.samples, interpolation='nearest')
